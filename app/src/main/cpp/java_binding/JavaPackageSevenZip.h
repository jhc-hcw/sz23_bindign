...
JT_BEGIN_CLASS(SEVEN_ZIP_PACKAGE_IMPL, InArchiveVolumeImpl)

    JT_FIELD(Long, jbindingSession)

    JT_FIELD(Long, Aresult)
...
JT_BEGIN_INTERFACE(SEVEN_ZIP_PACKAGE, IDirItemCallback)
        // long scanError(String path,int systemError);
    JT_INTERFACE_METHOD(Int, scanError, JT_STRING(total, JT_INT(systemError, _)))

        // long scanProgress(String path,boolean isDir);
    JT_INTERFACE_METHOD(Int, scanProgress, JT_STRING(total, JT_BOOLEAN(isDir, _)))
JT_END_INTERFACE


JT_BEGIN_INTERFACE(SEVEN_ZIP_PACKAGE, IExtractCallbackUI)

        //void setTotal(long size);
    JT_INTERFACE_METHOD(Void, setTotal, JT_LONG(size, _))

        //void setCompleted(long size);
    JT_INTERFACE_METHOD(Void, setCompleted, JT_LONG(size, _))

    JT_INTERFACE_METHOD(Void, needPassword, _)

        //int openCheckBreak();
    JT_INTERFACE_METHOD(Int, openCheckBreak, _)

        //int openSetTotal(long files, long bytes);
    JT_INTERFACE_METHOD(Int, openSetTotal, JT_LONG(files, JT_LONG(byts, _)))

//	int openSetCompleted(long files, long bytes);
    JT_INTERFACE_METHOD(Int, openSetCompleted, JT_LONG(files, JT_LONG(bytes, _)))
//
//	int openFinished();
    JT_INTERFACE_METHOD(Int, openFinished, _)
//
//	int beforeOpen(String name, boolean testMode);
    JT_INTERFACE_METHOD(Int, beforeOpen, JT_STRING(name, JT_BOOLEAN(testMode, _)))
//
//	int openResult(String name, int result);
    JT_INTERFACE_METHOD(Int, openResult, JT_STRING(name, JT_INT(result, _)))

//  int thereAreNoFiles();
    JT_INTERFACE_METHOD(Int, thereAreNoFiles, _)
//
//	int extractResult(int result);
    JT_INTERFACE_METHOD(Int, extractResult, JT_INT(result, _))

//        void scanProgress(DirItemStat st,String path,boolean isDir);
    JT_INTERFACE_METHOD(Void, scanProgress, JT_DIRITEM(st, JT_STRING(path, JT_BOOLEAN(idDir, _))))
JT_END_INTERFACE


JT_BEGIN_INTERFACE(SEVEN_ZIP_PACKAGE, IUpdateOpenCallback)
        //void openSetTotal(long files, long bytes);
    JT_INTERFACE_METHOD(Void, openSetTotal, JT_LONG(files, JT_LONG(bytes, _)))
        //void openSetCompleted(long files, long bytes);
    JT_INTERFACE_METHOD(Void, openSetCompleted, JT_LONG(files, JT_LONG(bytes, _)))
        //void openFinished();
    JT_INTERFACE_METHOD(Void, openFinished, _)
JT_END_INTERFACE


JT_BEGIN_INTERFACE(SEVEN_ZIP_PACKAGE, IUpdateCallback)
        //void setTotal(long size);
    JT_INTERFACE_METHOD(Void, setTotal, JT_LONG(size, _))
        //void setCompleted(long completedValue);
    JT_INTERFACE_METHOD(Void, setCompleted, JT_LONG(completedValue, _))
        //void setRatioInfo(long inSize, long outSize);
    JT_INTERFACE_METHOD(Void, setRatioInfo, JT_LONG(inSize, JT_LONG(outSize, _)))
        //void openResult(String name, int result);
    JT_INTERFACE_METHOD(Void, openResult, JT_STRING(name, JT_INT(result, _)))
        //void scanProgress(DirItemStat st, String path, boolean isDir);
//        long numDir,
//        long numFile,
//        long numAltStreams,JT_STRING(path, JT_BOOLEAN(isDir, _))
    JT_INTERFACE_METHOD(Void, scanProgress, JT_LONG(numDir, JT_LONG(numFile, JT_LONG(numAltStreams,
                                                                                     JT_STRING(path,
                                                                                               JT_BOOLEAN(
                                                                                                       isDir,
                                                                                                       _))))))
        //void finishScanning(DirItemStat st);
    JT_INTERFACE_METHOD(Void, finishScanning,
                        JT_LONG(numDir, JT_LONG(numFile, JT_LONG(numAltStreams, _))))
        //void startOpenArchive(String name);
    JT_INTERFACE_METHOD(Void, startOpenArchive, JT_STRING(name, _))
        //void startArchive(String name, boolean updating);
    JT_INTERFACE_METHOD(Void, startArchive, JT_STRING(name, JT_BOOLEAN(updating, _)))
        //void finishArchive(long size);
    JT_INTERFACE_METHOD(Void, finishArchive, JT_LONG(size, _))
        //void deletingAfterArchiving(String path, boolean isDir);
    JT_INTERFACE_METHOD(Void, deletingAfterArchiving, JT_STRING(path, JT_BOOLEAN(isDir, _)))

    JT_INTERFACE_METHOD(Int, CheckBreak, _)
JT_END_INTERFACE

JT_BEGIN_CLASS(SEVEN_ZIP_PACKAGE, DirItemStat)
//    public long numDirs;
    JT_FIELD(Long, numDirs)
//    public long numFiles;
    JT_FIELD(Long, numFiles)
//    public long numAltStreams;
    JT_FIELD(Long, numAltStreams)
//    public long fileSizes;
    JT_FIELD(Long, fileSizes)
//    public long altStreamSize;
    JT_FIELD(Long, altStreamSize)
//    public long numErrorsl;
    JT_FIELD(Long, numErrorsl)
JT_END_INTERFACE
