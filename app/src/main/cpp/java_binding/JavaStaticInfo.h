...
#define JT_DIRITEM(name, param_spec)                                                             \
            JT_PARAM(Object, "Lnet.sf.sevenzipjbinding.DirItemStat;", name, param_spec)

//#define JT_STRING_INT(name,param_spec) \
//            JT_STRING(path,Int,"I",name,param_spec)
// ------------------------------
...
namespace jni {
    ...
    public:
        ...
        static T *_getInstanceFromObject(JNIEnv *env, jobject jobject) {
            ...
            jLogStr("in _getInstanceFromObject");

#ifdef __ANDROID_API__
            jclass jobjectClass = findClass(env,
                                            T::_getName()); //每一个继承jinterface的都会有getName静态方法获取包名
            jLogStr(T::_getName());
#else
        ...
        static T *_getInstance(JNIEnv *env, jclass objectClass) {
            ...
            _jinterfaceMap.add(objectClass, newInstance);//合理的。key是jobj，value是绑定了jobj的c++回调
